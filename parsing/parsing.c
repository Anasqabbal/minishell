...
// static void	free_it(char **arg, int count)
// {
// 	int	i;
...
void parse_redirections(char **tokens, t_prs *node)
{
    ...
    while (y < len)
    {
        if (tokens[y][0] == '>')
        {
            add_to_struct(&node->out, tokens[y]);
        	add_to_struct(&node->out_f, tokens[y + 1]);
        }
        else if (tokens[y][0] == '<')
        {
            add_to_struct(&node->in, tokens[y]);
            add_to_struct(&node->in_f, tokens[y + 1]);
        }
        else if ((tokens[y][0] != '>' && tokens[y][0] != '<' && tokens[y][0] != '-') && (tokens[y - 1][0] != '<'  && tokens[y - 1][0] != '>' ))
        {
...
int check_syntax(char *input, t_list *env, t_prs **struc)
{
	...
	if(!check_qts(cmd, &opp, &qts))
		return (0);
	(*struc) =  pipe_split(cmd, opp, qts, env);
...
// 		if(!envi)
// 			return(1);
	// if (!check_syntax(input, envi, stru))
